...
% Last Modified by GUIDE v2.5 13-Sep-2011 15:44:38
...
handles.configList={'sectorSel' 'appMode' 'acquireSampleNum' 'acquireDataNum' ...
    'dataAverage' 'dataShowAll' 'acquireDataRange' 'simul'};
...
txtList={'girdOff' 'girdSlope' 'quadOff' 'undOff' 'bpmOff' 'bpmNoise' ...
    'beamJitt' 'beamAngle' 'enRange' 'nEnergy' 'girdBack' 'fitScale' ...
    'fitSVDRatio' 'corrGain' 'girderNum'};
for tag=txtList
    txtControl(hObject,handles,tag{:},[]);
end
boxList=[strcat('use',{'GirdOff' 'GirdSlope' 'QuadOff' 'UndOff' 'BPMOff' ...
    'BPMNoise' 'BeamJitt' 'GirdBack' 'Und' 'Stray'}) ...
    strcat('fit',{'BPMLin' 'BPMMin' 'QuadMin' 'QuadLin' 'CorrAbs'}) {'noEPlusCorr'}];
for tag=boxList
...
set(handles.acquireStart_btn,'Enable', ...
    state{(handles.appTask > 0 | handles.acquireDataNum == 1 | ispc)+1});
if ~ispc
%    set([handles.acquireStart_btn handles.acquireEnergySet_btn ...
%        handles.acquireEnergyReset_btn],'Enable',state{~val+1});
end
set([handles.simul_pan handles.acquireEnergySet_btn handles.acquireEnergyReset_btn],'Visible',state{(~val | ispc)+1});
%set([handles.setKlysKick_btn handles.klysName_txt handles.klysNameLabel_txt ...
%    handles.doLEM_box],'Visible',state{(~val | ispc)+1});
...
handles=gui_popupMenuControl(hObject,handles,'sectorSel',val, ...
    {'UND' 'L2' 'LI25 LI26' 'LI27 LI28' 'LI29 LI30' 'BC2_L3END' 'BSY' 'LI30 BSY' ...
     'BSY_End LTU0 LTU1' 'BSY LTU0 LTU1' 'LI24_End BC2_L3END BSY LTU0 LTU1' ...
     'UND_Launch' 'L2 L3 50B1_BSY LTU0 LTU1 UND1' 'UND_Launch2'}, ...
    {'UND' 'L2' '25-26'     '27-28'     '29-30'     'L3'        'BSY' '30-BSY'   ...
     'LTU'               'BSY-LTU'       'L3-LTU'                           ...
     'UND_Launch' 'L2-L3-LTU'                     'UND_Launch2'});
if cancd, return, end
...
set(handles.acquireDataRangeUnits_txt,'String',units, ...
    'Visible',state{(handles.appTask == 4 | handles.appTask == 0 & ispc)+1});
handles=acquireReset(hObject,handles);
...
[handles,cancd,val]=gui_dataRemove(hObject,handles,val);
vis=handles.appTask == 4 | handles.appTask == 0 & ispc;
...
if handles.appTask == 2 || (handles.appTask == 0 && ~ispc), str={};end
...
if handles.appTask == 3
    ...
    lcaPutNoWait(strcat(undPV,':TMXPOSC'),val);if ~ispc, pause(1.);end
...
if handles.fitGirderMove || handles.fitCorr*0 || handles.fitDiff
...
%    xMeas(:,:,:,2:end)=diff(xMeas,1,4);
%    xMeasStd(:,:,:,2:end)=sqrt(xMeasStd(:,:,:,2:end).^2+xMeasStd(:,:,:,1:end-1).^2);
end
...
%use=find(~cellfun('isempty',data.R));
for k=1:nEn
...
if ~handles.dataAverage
    for k=1:nEn*handles.acquireSampleNum
        r(:,k,[(1:nInit)+nInit*(k-1) nInit*nEn+(1:nPar-nInit)])=data.R{fix((use(k)-1)/handles.acquireSampleNum)+1};
    end
end
...
if sum(data.status) == 2 && handles.fitGirderMove && handles.appTask ~= 3
    use=1;
    iGird=handles.simul.girderNum;
...
elseif sum(data.status) >= 2 && handles.fitGirderMove
...
elseif sum(data.status) == 1 && handles.fitCorr
...
%        r(:,k,[(1:4)+4*(k-1) 4*nEn+(1:nPar-4)])=diff(R(:,:,k:k+1),1,3);
    ...
    if 0
        isBSY=strcmp(handles.static.bpmList,'BPMBSY29');
...
elseif sum(data.status) > 2 && handles.appTask == 4
...
%        bp=bp0+.254*cosd(val(j));bp=bp0;
        corrX(:,j)=squeeze(f.corrOff(1,isFBX));%*bp/bp0;
        corrY(:,j)=squeeze(f.corrOff(2,isFBY));%*bp/bp0;
        corrStdX(:,j)=squeeze(f.corrOffStd(1,isFBX));
        corrStdY(:,j)=squeeze(f.corrOffStd(2,isFBY));
...
if opts.use.quad || opts.use.BPM
    bba_plotOffset(handles.static,{f.quadOff f.quadOffStd},{f.bpmOff f.bpmOffStd},handles.appMode,'title',tle);
end
if opts.use.corr
...
%figure(1);set(subplot(2,1,1),'XLim',[550 950]);set(subplot(2,1,2),'XLim',[550 950]);
...
if ~strcmp(handles.sectorSel,'UND'), data.static=handles.static;end
...
function applyCorrs_btn_Callback(hObject, eventdata, handles)
%correct orbit using the corrector results from bba codes.
...
set([handles.updateMovePV_btn handles.zeroMovePV_btn handles.orbitCorr_pan handles.undInOut_pan handles.klysKick_pan],'Visible','off');
...
set(handles.orbitCorr_pan,'Visible','on');
set([handles.stdzMagnets_btn handles.updateMovePV_btn handles.zeroMovePV_btn handles.correction_pan handles.undInOut_pan handles.klysKick_pan],'Visible','off');
...
set([handles.stdzMagnets_btn handles.correction_pan handles.orbitCorr_pan handles.klysKick_pan],'Visible','off');
...
set([handles.stdzMagnets_btn handles.correction_pan handles.undInOut_pan handles.orbitCorr_pan],'Visible','off');
...
function setUndBBA_btn_CreateFcn(hObject, eventdata, handles)
% hObject    handle to setUndBBA_btn (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
