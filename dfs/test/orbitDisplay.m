...
% Get BPM list.
[nAll,z,x]=getBPMs(opts.sector,[hl1 hl2 hl3]);
...
while opts.delay && ishandle(1)
    [data,ts]=lcaGetSmart(strcat(nAll,opts.eDef{1}));
    data=reshape(data,[],3);
    x=[0;1;NaN]*data(:,1)';
    y=[0;1;NaN]*data(:,2)';
    t=[0;1;NaN]*data(:,3)';
    ...
    pause(opts.delay);
    drawnow;
    if ~ishandle(1), return, end
    if ~opts.gui, continue, end
    handles=guidata(1);
    if ~isequal(opts.sector,handles.sector)
        [nAll,z,x]=getBPMs(handles.sector,[hl1 hl2 hl3]);
        [x2,y2,t2]=deal(x*NaN);
        if ~isempty(opts.zLim), set([h1 h2 h3],'XLim',opts.zLim);end
        set([h1 h2],'XTickLabel','','XLim',get(h3,'XLim'));
    end
    opts.eDef=handles.eDef;
    opts.delay=handles.delay;
    opts.xLim=handles.xLim;
    opts.chargeLim=handles.chargeLim;
...
function [nAll, z, x] = getBPMs(sector, hl)
