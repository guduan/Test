...
if epicsSimul_status
    if ~noPDES
        ...
        lcaPut(namePACT(~is.L23),pDes(~is.L23));
    end
...
if any(is.L2 & ~isGold & isempty(ds))
    ...
    pL2=lcaGetSmart([{nameL2Ref};name(find(is.L2 & ~isGold,1))]);
end
...
if any(is.KLY)
    ...
    if any(isTrim), lcaPut(strcat(name(isTrim),':TRIMPHAS'),1);end
    pAct(is.KLY)=phaseSettle(name(is.KLY),pDes(is.KLY),pTol,max(1,nTry*trim));
end

% Set PACT for no PAD devices and global L2/3
...
if ~ispc, disp_log(str);else disp(str);end
